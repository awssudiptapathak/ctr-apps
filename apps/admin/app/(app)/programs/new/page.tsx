...
export default function NewProgramPage() {
  ...
  useEffect(() => {
    api
      ...
      .then(({ events }) => {
        setEvents(events);
        setForm((current) => ({ ...current, eventId: current.eventId || events[0]?.id || '' }));
      })
      .catch((e: any) => e.status === 401 && router.push('/login'));
  }, [router]);
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', background: 'rgba(20, 11, 13, 0.8)', border: '1px solid rgba(249,210,122,0.35)', borderRadius: 22, padding: '1.5rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
          <div>
            ...
            <h1 style={{ margin: '0.5rem 0 0', fontSize: 'clamp(2rem, 4vw, 3rem)' }}>New program</h1>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '1rem' }}>
          <label style={{ display: 'grid', gap: '0.45rem', color: '#f6e7c0' }}>
            <span>Event</span>
            <select value={form.eventId} onChange={(event) => updateField('eventId', event.target.value)} style={fieldStyle}>
              {events.map((event) => (
